/**
 ...
 - [F01] Quick examples of filter, map, reduce, sort
 - [F02] Company structure output
 - [F03] All employees grouped by department
 - [F04] Total sales per employee
 - [F05] The top salesperson per department
 - [F06] Employees with no sales
 - [F07] Departments with the most employees
 - [F08] Top 3 sales by amount (across all employees)
 - [F09] Sales Leaderboard
 */
...
// - [F01] Quick examples of filter, map, reduce, sort
print ("# [F01] - Examples of filter, map, reduce, sort\n")
printQuickFunctionalExamples(for: company)

// - [F02] Company structure output
print ("# [F02] - Company structure\n")
printCompanyStructure(for: company)

print ("\n--------------------\n")

// - [F03] All employees grouped by department
print ("# [F03] - All employees grouped by department \n")
showAllEmployeesGroupedByDepartment(in: company)

print ("\n--------------------\n")

// - [F04] Total sales per employee
print ("# [F04] Total sales per employee \n")
showTotalSalesPerEmployee(in: company)

print ("\n--------------------\n")

// - [F05] The top salesperson per department
print ("# [F05] The top salesperson per department \n")
showTopSalesPersonPerDepartment(in: company)
    
print ("\n--------------------\n")

// - [F06] Employees with no sales
print ("# [F06] Employees with no sales \n")
showAllEmployeesWithNoSales(in: company)

print ("\n--------------------\n")

// - [F07] Departments with the most employees
print ("# [F07] Departments with the most employees \n")
showAllDepartmentsWithMostEmployees(in: company)

print ("\n--------------------\n")

// - [F08] Top 3 sales by amount (across all employees)
print ("# [F08] Top 3 sales by amount (across all employees) \n")
showTop3SalesByAmount(in: company)

print ("\n--------------------\n")

// - [F09] Sales Leaderboard
print ("# [F09] Sales Leaderboard \n")
...
func printQuickFunctionalExamples(for company: Company) {
    ...
    // Filter all sales made, over $1000 by Employee; sort them.
    let salesFilter = company.departments
        .flatMap { $0.employees } /// Flattens nested collections of employees
        .flatMap { $0.sales } /// Flattens nested collections of sales
        .filter { $0.amount > 1000 } /// get me amounts  > 1000
    ...
    /// Example use: Get names of all employees:
    let mapEG = company.employees.map { $0.name }
    
    print ("\nGet all names of employees: \(mapEG)\n")
    
    // `compactMap`
    /// Usage: Transforms elements AND removes nils from the result.
    /// Example: Get all employees who have departments:
    let compactMapEG = company.employees
        .compactMap { $0.department }
    
    print ("Get all employees who have departments: \(compactMapEG)\n")
    
    // `flatMap`
    /// Usage: Flattens nested collections into a single array.
    /// Example: Get all sales from all employees:
    let flatMapEG = company.departments.flatMap { $0.employees }.flatMap { $0.sales }
    ...
    // `zip`
    /// Usage: Combines two collections into pairs. Use when: you want to pair items from two lists
    /// Example: Pair employee names with their total sales:
    
    let zipEG = zip(
        company.employees.map { $0.name },
        company.employees.map { $0.totalSales }
    )
...
/// Show the top 3 sales (individual Sale objects), across all employees, ranked by amount.
func showTop3SalesByAmount(in company: Company) {
    
    // Gather all sales from all employees
        let allSales = company.departments
            .flatMap { $0.employees }
            .flatMap { $0.sales }
    
    let topSales = allSales
           .sorted(by: { $0.amount > $1.amount })
           .prefix(3) // returns the first 3 elements of a sequence.
...
func showSalesLeaderboard(for company: Company) {
    ...
    /// total up (reduce) the sales, then sort by total sales
    let ranked = allEmployees
           .map { ($0, $0.sales.reduce(0.0) { $0 + $1.amount }) }
           .sorted { $0.1 > $1.1 } // Sort by total sales
